...
export class CsvEditor {
  ...
  /**
   * Reorder columns in CSV dataset according to an ordered list of column names
   * @param {string} csvText
   * @param {Array<string>} newColumnOrder
   * @param {object} [options]
   * @returns {string}
   */
  static reorderColumns(csvText, newColumnOrder, options = {}) {
    ...
    // Include specified columns first, followed by any unspecified remaining columns
    const remainingCols = currentCols.filter(c => !requestedCols.has(c));
    const finalCols = [...newColumnOrder.filter(c => currentCols.includes(c)), ...remainingCols];

    const reorderedRecords = records.map(row => {
      const obj = {};
    ...
    return CsvSerializer.serialize(reorderedRecords, { ...options, columns: finalCols });
  }

  /**
   * Reorder rows by an array of 0-based row indices
   * @param {string} csvText
   * @param {Array<number>} newRowOrder
   * @param {object} [options]
   * @returns {string}
   */
  static reorderRows(csvText, newRowOrder, options = {}) {
    const records = CsvParser.parse(csvText, { ...options, asObjects: true });
    const reordered = [];
    for (const idx of newRowOrder) {
      if (idx >= 0 && idx < records.length) {
        reordered.push(records[idx]);
      }
    }
    return CsvSerializer.serialize(reordered, options);
  }
}
